...
export default function KeyResultsStrip() {
  return (
    <section className="relative py-14 sm:py-20">
      <div className="mx-auto max-w-7xl lg:max-w-[1600px] px-4 sm:px-6">
        <div className="mb-8 sm:mb-10 text-center">
          <p className="text-[0.7rem] sm:text-xs font-semibold uppercase tracking-[0.25em] text-cyan-300/70">
            Outcomes
          </p>
        ...
        <div className="grid gap-5 sm:gap-6 md:grid-cols-3">
          {stats.map((stat) => (
            <div key={stat.label} className="group relative">
              ...
              <div
                className={`pointer-events-none absolute -inset-[1px] rounded-[2rem] sm:rounded-[2.4rem] opacity-0 blur-xl sm:blur-2xl transition duration-500 ${stat.glowClass}`}
              />

              {/* OUTER FRAME */}
              <div className="relative h-full rounded-[1.9rem] sm:rounded-[2.3rem] border border-slate-800/60 bg-slate-950/40 p-[5px] sm:p-1 backdrop-blur-2xl shadow-[0_0_0_1px_rgba(15,23,42,0.9)] transition duration-300 group-hover:border-slate-100/30">
                {/* INNER TILE */}
                <div
                  className="relative flex h-full flex-col justify-between rounded-[1.6rem] sm:rounded-[2rem]
                             border border-slate-900/80 bg-slate-950/85 px-5 py-5 sm:px-8 sm:py-7
                             shadow-[0_18px_40px_rgba(15,23,42,0.9)] sm:shadow-[0_18px_50px_rgba(15,23,42,0.95)]
                ...
                >
                  ...
                  <div className="relative z-10">
                    {/* icon pill */}
                    <div
                      className="inline-flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-2xl
                                 bg-slate-900/90 ring-1 ring-pink-500/40
                                 shadow-[0_0_20px_rgba(236,72,153,0.45)] sm:shadow-[0_0_24px_rgba(236,72,153,0.45)]
                    ...
                    {/* primary metric */}
                    <div className="mt-5 sm:mt-6 mb-1 flex items-baseline gap-2">
                      <span className="text-2xl sm:text-4xl font-semibold tracking-tight text-slate-50">
                        {stat.primary}
                      </span>
                      <span className="text-xl sm:text-3xl font-semibold text-pink-400">
                        {stat.accent}
                      </span>
                    </div>
